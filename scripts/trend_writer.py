...
5. **근거 추적성**: 핵심 기술 주장·수치는 선정 글감이나 이름 있는 보조 레퍼런스에 근거를 두고, 출처 없는 단정은 쓰지 말 것
6. **재현 가능한 산출물**: how-to·기술 설명 글이면 재현 가능한 구체 산출물(명령어, 코드/설정 블록, 버전·날짜가 박힌 절차, 또는 구체적 측정 수치) 최소 1개를 반드시 포함
